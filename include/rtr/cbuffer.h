...
#include "rtr/qtool.h"
#include "rtr/platform.h"		/* RDEVDIFFOK */
...
typedef struct rtr_bufferpool {
	...
	int				numBufs;	/* Current number of buffers */
...
#define RTBUFFERPOOLLOCK(pool,mterr) \
	if ((pool)->mutex) RTR_MUTEX_LOCK(*((pool)->mutex),mterr);

#define RTBUFFERPOOLUNLOCK(pool,mterr) \
	if ((pool)->mutex) RTR_MUTEX_UNLOCK(*((pool)->mutex),mterr);

typedef struct rtr_datab {
	rtr_qtool_head	link;		/* Internal, never touch */
...
typedef struct rtr_msgb {
	rtr_qtool_head	link;		/* May be used for list operations by owner */
	rtr_bufferpool_t	*pool;		/* Pool associated with this message */
	void			*internal;	/* Internal, never touch */
