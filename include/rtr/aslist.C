...
void RTRAscSortList<T,L>::extend(T *object)
{
...
template<class T,class L>
void RTRAscSortList<T,L>::extendFromBehind(T *object)
{
	RTPRECONDITION ( object );
	T *cur;
...
template<class T,class L>
void RTRAscSortList<T,L>::deleteContents()
{
	T *cur;
...
template<class T,class L>
void RTRAscSortList<T,L>::wipeOut()
{
	while (!empty())
		removeFirst();
	RTPOSTCONDITION(empty());
