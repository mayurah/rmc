...
//	rtr_pipe_create:Pipe initialization
...
RTR_C_INLINE void rtr_pipe_init(rtr_pipe* _pipe) {
	_pipe->_fds[0] = -1;
	_pipe->_fds[1] = -1; 
	_pipe->_initialized = 0;
}

RTR_C_INLINE int rtr_pipe_get_read_fd(rtr_pipe* _pipe) {
...
RTR_C_INLINE int rtr_pipe_get_write_fd(rtr_pipe* _pipe) {
	if ( _pipe->_initialized)
		return _pipe->_fds[1];
	else
		return -1;
...
// Closes the pipe.
RTR_C_INLINE void rtr_pipe_close(rtr_pipe* _pipe) {
	if( _pipe->_initialized )
	{
		if(_pipe->_fds[0] != -1 ) {
#ifdef _WIN32
...
// Returns 1 if the pipe is sucessfully created, and 0 if not.
RTR_C_INLINE int rtr_pipe_create(rtr_pipe* _pipe) {
#ifdef _WIN32
	int port;
	int flag;
	int ioctlRetVal;
	...
	while ( status == -1 )
	{
		...
		serverFD = (int)socket( AF_INET, SOCK_STREAM, PF_UNSPEC );
		...
		_pipe->_fds[1] = (int)socket( AF_INET, SOCK_STREAM, PF_UNSPEC );
		...
		_pipe->_fds[0] = (int)accept( serverFD, (struct sockaddr *)&cliSAddr, &SAddrSize );
...
// Attempts to write lengh bytes from the buffer to the pipe.
// Returns either the number of bytes added to the pipe, or a POSIX/Winsock error.
RTR_C_INLINE int rtr_pipe_write(rtr_pipe* _pipe, const void * buffer, size_t length ) {
#ifdef _WIN32
	int retVal;
	retVal = (int)send( _pipe->_fds[1], ( const char *)buffer, (int)length, 0 );
...
// Reads length bytes from the pipe, and places it into the buffer.
// Returns either the length read from the pipe, or a POSIX/Winsock error value.
RTR_C_INLINE int rtr_pipe_read(rtr_pipe* _pipe, void * buffer, size_t length ) {
#ifdef _WIN32
	...
	return ((int)(retVal <= 0 ? retVal : length));
#else
