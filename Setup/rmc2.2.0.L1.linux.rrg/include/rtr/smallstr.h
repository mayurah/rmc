...
#ifndef __RTR_SMALL_STRING
#define __RTR_SMALL_STRING
...
// Description:
// See Also:
//	RTRString, RTRExternalValue, RTRListOfExternalValue
//

class RTRSmallString
{
#ifndef _RDEV_NO_STL_
friend std::ostream& operator<<(std::ostream& os, RTRSmallString& aSmallString);
#else
public:
void print(FILE*);
#endif
...
// Constructors
	...
	RTRSmallString(int len);
		// Create a small buffer of capacity `len'
		// REQUIRE : len < 256
        // _TAG Constructor

	RTRSmallString(	const char* n,
					int num_bytes,
					int cap);
	...
	RTRSmallString(const RTRSmallString &aSmallString);
		// Copy constructor
        // _TAG Constructor
...
// Equality comparisons
	RTRBOOL operator==(const RTRSmallString& aSmallString);
...
// Assignment
	RTRSmallString&	operator=(const RTRSmallString& aSmallString);
		// _TAG03 Assignment
...
public:
	...
	static unsigned long hashKeySmallString( RTRSmallString *name );
...
// Constructors
	RTRSmallString(int num_bytes, const char * n, int cap);

};

inline int RTRSmallString::length() const {return( _length );};
inline int RTRSmallString::capacity() const {return( _capacity );};

inline const char *RTRSmallString::to_c() const
{
...
inline RTRSmallString::operator const char*() const {return to_c();};

inline void RTRSmallString::set( const char *cstr, int p1, int p2 )
{
	RTPRECONDITION( p1 >= 1 );
...
inline void RTRSmallString::set( const char *cstr, int len )
{
...
inline void RTRSmallString::force( const char *cstr, int p1, int p2 )
{
	RTPRECONDITION( p1 >= 1 );
	RTPRECONDITION( p2 >= p1 );
	_length = p2-p1+1;
...
inline void RTRSmallString::force( const char *cstr, int len )
{
	_length = len;
	grow( _length );
